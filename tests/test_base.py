...
from winloop._testbase import AIOTestCase, UVTestCase
...
class _TestBase:
    ...
    def test_calls_debug_reporting(self):
        def run_test(debug, meth, stack_adj):
            ...
            self.assertTrue(context["message"].startswith("Exception in callback"))
    ...
    @unittest.skipIf(sys.version_info == (3, 12), "uvloop also suffers from the same problem it's not just a winloop problem")
    ...
    def test_call_at(self):
        if os.environ.get("TRAVIS_OS_NAME") or os.environ.get("GITHUB_WORKFLOW"):
    ...
    def test_default_exc_handler_callback(self):
        ...
        with mock.patch.object(logger, "error") as log:
            ...
            log.assert_called_with(
                self.mock_pattern("Exception in callback.*zero"), exc_info=mock.ANY
            )
    ...
    def test_set_exc_handler_custom(self):
        ...
        self.assertRegex(errors[-1]["message"], "Exception in callback.*zero_error")
    ...
    def test_call_soon_threadsafe_safety(self):
        ...
        def cb():
            ...
            if counter[0] < ITERATIONS - 512:
                ...
                self.loop.call_later(0.0005 + random.random() * 0.0005, h.cancel)
